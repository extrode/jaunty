...
public static class SqlDialectFactory
{
    ...
    /// <c>_ =&gt; new SqlServerDialect() // Default to SQL Server</c>, so any connection type Extrode.Jaunty
    ...
    /// the switch, so a decorated <c>SqliteConnection</c> quietly produced SQL Server SQL. Extrode.Jaunty
    ...
    private static Func<IDbConnection, IDbConnection?>? BuildInnerConnectionAccessor(Type connectionType)
    {
        ...
        // AOT-SAFE: a best-effort probe on a decorator supplied by the caller, not on a Extrode.Jaunty type.
