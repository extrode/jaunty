...
internal static class WriteParameterCache<T> where T : new()
{
    ...
    /// binders-from-a-different-Extrode.Jaunty-version as a real deployment shape.
    /// </para>
    /// <para>
            ...
            for (int i = 0; i < getters.Length; i++)
            {
                ...
                if (pc[i] is not IDataParameter parameter)
                    throw new InvalidOperationException(
                        ...
                        $"'{columnNames[i]}' was expected. Extrode.Jaunty binds bulk values by position; an entry it cannot " +
    ...
    private static EntityMetadata? ResolveMetadata()
    {
        ...
        // 2. Fallback to extension hook (Extrode.Jaunty.Extensions.Reflection)
