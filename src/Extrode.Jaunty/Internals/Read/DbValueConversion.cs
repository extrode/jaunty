...
/// <remarks>
/// <para>
/// AUD-R26-062. Extrode.Jaunty had three of these and they disagreed. <c>DbValueConverter.ChangeType</c>
/// (Extrode.Jaunty.Extensions.Reflection), <c>GroupedJoinedResultMapper.ConvertColumnValue</c>
/// (Extrode.Jaunty.Fluent) and <c>ScalarConverter&lt;T&gt;.Convert</c> (here) all pinned
...
internal static class DbValueConversion
{
    ...
    private static InvalidCastException NoConversion(object value, Type targetType) =>
        new($"Cannot convert a value of type '{value.GetType().FullName}' to '{targetType.FullName}'. " +
            ...
            "convert it yourself - the byte order of a binary GUID is provider-specific and Extrode.Jaunty " +
            "will not guess it.");
}
