namespace Extrode.Jaunty.Interceptors;

/// <summary>
/// Defines a hook for intercepting command execution in Extrode.Jaunty.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>Interceptors are not the only way to observe.</b> Subscribing to the <c>"Extrode.Jaunty"</c>
...
/// </para>
/// <para>
/// <b>Coverage across assemblies.</b> The fluent API (<c>Extrode.Jaunty.Fluent</c>) and the DuckDB flat-file
/// API (<c>Extrode.Jaunty.FlatFiles.DuckDB</c>) route their reads and writes through the same pipeline as
/// core. Two DuckDB areas do not, deliberately: the session setup statements that install and load
/// DuckDB extensions and probe a source's schema, which are Extrode.Jaunty's own plumbing rather than the
