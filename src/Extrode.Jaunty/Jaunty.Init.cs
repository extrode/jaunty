...
public static partial class Jaunty
{
    ...
    /// <summary>
    /// Attempts to load and initialize reflection-based mapping from Extrode.Jaunty.Extensions.Reflection.
    ...
    /// </para>
    /// <list type="number">
    /// <item><description>Exclude Extrode.Jaunty.Extensions.Reflection from trimming (recommended if using special types)</description></item>
    ...
    internal static Exception? TryEnableReflectionMapping(Func<AssemblyName, Assembly> load)
    {
        try
        {
            ...
            var assembly = load(new AssemblyName("Extrode.Jaunty.Extensions.Reflection"));

            Type? type = assembly.GetType("Extrode.Jaunty.Extensions.Reflection.JauntyReflectionExtensions");
            // AOT-SAFE: optional probe for Extrode.Jaunty.Extensions.Reflection; absent or trimmed is the expected source-gen-only case and returns null unrecorded - only an unexpected exception reaches the catch-all that sets ReflectionMappingInitializationError.
            ...
            // any Extrode.Jaunty static member, which is normally the caller's first query. Every hook below
        ...
        catch (Exception ex)
        {
            ...
            // TypeInitializationException on first touch of any Extrode.Jaunty API - but recorded, so the
