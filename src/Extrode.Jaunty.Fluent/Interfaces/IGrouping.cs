...
/// <c>Extrode.Jaunty.Fluent.IGrouping&lt;TKey, T&gt;</c>, or add a <c>using</c> alias) is not discoverable
/// from the message. No in-repo site can see it: every one sits inside or under the
/// <c>Extrode.Jaunty.Fluent</c> namespace, where enclosing-namespace lookup beats the using directive.
