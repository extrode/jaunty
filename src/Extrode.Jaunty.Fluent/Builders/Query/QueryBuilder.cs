    ...
    where T : new()
{
    ...
    private void ThrowIfPagedBeforeJoin()
    {
        if (_take.HasValue || _skip.HasValue)
        {
            throw new NotSupportedException(
                "Take/Skip applied before a join are not carried into the joined query. Extrode.Jaunty " +
    ...
    private void ThrowIfPagedBeforeGroupBy()
    {
        ...
        {
            throw new NotSupportedException(
                "Take/Skip applied before a GroupBy are not carried into the grouped query. Extrode.Jaunty " +
    ...
    /// across every dialect Extrode.Jaunty targets - so <see cref="AppendAliasedTable"/> is deliberately
    ...
    private int ExecuteNonQueryDirect(string sql, CommandOptions options)
    {
        ...
        try
        {
            ...
            // AsyncTransactionValidator first (mirroring Extrode.Jaunty core's GetByIdSimpleCoreDirect) so an
            // incompatible transaction gets Extrode.Jaunty's clear ArgumentException instead.
            if (options.Transaction is not null)
            {
                command.Transaction = _connection is System.Data.Common.DbConnection
                    ? global::Extrode.Jaunty.AsyncTransactionValidator.RequireDbTransaction(options.Transaction)
                    : options.Transaction;
            }
    ...
    private async ValueTask<int> ExecuteNonQueryDirectAsync(string sql, CommandOptions options, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (options.Transaction is not null)
                command.Transaction = global::Extrode.Jaunty.AsyncTransactionValidator.RequireDbTransaction(options.Transaction);
