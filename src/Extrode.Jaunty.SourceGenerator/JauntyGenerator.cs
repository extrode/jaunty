...
namespace Extrode.Jaunty.SourceGenerator;
...
public partial class JauntyGenerator : IIncrementalGenerator
{
    ...
    /// <remarks>
    /// <para>
    /// A source-generated entity implements <c>IGeneratedAccessors&lt;T&gt;</c>, so Extrode.Jaunty takes its
    ...
    private static readonly DiagnosticDescriptor PropertyDroppedByGeneratorDescriptor = new(
        ...
        messageFormat: "Property '{0}.{1}' is not mapped by the generated mapper because {2}, but Extrode.Jaunty's reflection mapper does map it - so referencing the generator package silently drops this column. Give it a plain accessible setter, or mark it [Ignore] to record that the exclusion is intended.",
        category: "JauntySourceGenerator",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true);

    /// <summary>The two <c>[Table]</c> attributes the generator recognizes, by metadata name.</summary>
    private const string JauntyTableAttribute = "Extrode.Jaunty.Attributes.TableAttribute";
    private const string DataAnnotationsTableAttribute = "System.ComponentModel.DataAnnotations.Schema.TableAttribute";

    // AUD-R35-070. The rest of the recognized set, listed Extrode.Jaunty-first because that is the
    // precedence MetadataBuilder applies: it asks for Extrode.Jaunty's typed attribute and only falls back
    ...
    private const string JauntyColumnAttribute = "Extrode.Jaunty.Attributes.ColumnAttribute";
    private const string DataAnnotationsColumnAttribute = "System.ComponentModel.DataAnnotations.Schema.ColumnAttribute";
    private const string JauntyKeyAttribute = "Extrode.Jaunty.Attributes.KeyAttribute";
    private const string DataAnnotationsKeyAttribute = "System.ComponentModel.DataAnnotations.KeyAttribute";
    private const string JauntyIgnoreAttribute = "Extrode.Jaunty.Attributes.IgnoreAttribute";
    private const string DataAnnotationsNotMappedAttribute = "System.ComponentModel.DataAnnotations.Schema.NotMappedAttribute";
    private const string JauntyDatabaseGeneratedAttribute = "Extrode.Jaunty.Attributes.DatabaseGeneratedAttribute";
    ...
    private const string JauntyEnumStorageAttribute = "Extrode.Jaunty.Attributes.EnumStorageAttribute";
    ...
    private const string JauntyInterfacesNamespace = "Extrode.Jaunty.Interfaces";
    ...
    /// <summary>
    /// Returns the class under the cursor when it implements <c>Extrode.Jaunty.Interfaces.IMapped&lt;T&gt;</c>
    ...
    /// most-derived-wins rule used by <c>MappedPropertyFilter</c> in Extrode.Jaunty.FlatFiles.DuckDB.
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// AUD-R25: two distinct routes reach the same hint name. A class carrying both a Extrode.Jaunty and a
    ...
    private static EntityModel BuildEntityModel(GeneratorAttributeSyntaxContext context)
    {
        ...
        foreach (IPropertySymbol? prop in allProperties)
        {
            ...
            if (dbGenAttr != null)
            {
                ...
                // Both Extrode.Jaunty and DataAnnotations use 1 for Identity, 2 for Computed
            ...
            // (e.g. a "Extrode.Jaunty.*" entity namespace colliding with the "Extrode.Jaunty" extension-methods
            ...
            // type keeps the model free of Extrode.Jaunty-assembly references.
    ...
    /// <summary>
    /// Generates the complete source text for a Extrode.Jaunty entity mapper partial class,
    ...
    static string GenerateMapper(EntityModel entity, Action<Diagnostic> reportDiagnostic)
    {
        ...
        sb.AppendLine("using Extrode.Jaunty.Interfaces;");
        sb.AppendLine();
        ...
        sb.AppendLine($"            bool {TypeHandlerFlagLocal} = global::Extrode.Jaunty.Core.GeneratedBindingSupport.HasHandlers;");
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        private static void AddEnumParam(IDbCommand cmd, IDataParameterCollection pc, string name, object? value, global::Extrode.Jaunty.Attributes.EnumStorage? storage)");
        sb.AppendLine("        {");
        sb.AppendLine("            var p = cmd.CreateParameter();");
        sb.AppendLine("            p.ParameterName = name;");
    ...
    /// carrying both Extrode.Jaunty's attribute and the DataAnnotations one resolves the same way
    ...
    /// <c>[Table]</c> attribute (Extrode.Jaunty native or DataAnnotations, matched by simple class
    /// name so either is accepted). Falls back to the class name when no attribute is present.
    /// Schema is read from either a positional second constructor argument (Extrode.Jaunty's
