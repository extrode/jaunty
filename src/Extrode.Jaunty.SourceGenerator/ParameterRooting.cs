...
/// <summary>
/// Spec 011. Emits trimmer-rooting calls for the parameter objects the consumer passes to Extrode.Jaunty, so
/// their property getters survive a trimmed or NativeAOT publish.
/// </summary>
/// <remarks>
/// <para>
/// Extrode.Jaunty reads a parameters object by reflecting over its public properties. Nothing in that path
...
/// often a type Extrode.Jaunty has never been told about.
/// </para>
/// <para>
...
public partial class JauntyGenerator
{
    ...
    /// <summary>
    /// Reported when a Extrode.Jaunty call site passes a parameters object whose type cannot be rooted from
    ...
    /// <summary>
    /// What one Extrode.Jaunty call site's parameters argument turned out to be: something to root, something
    ...
    /// <see langword="null"/> when it is not a Extrode.Jaunty parameter-binding call or needs nothing.
    /// </summary>
    private static ParameterSite? FindParameterSite(GeneratorSyntaxContext ctx)
    {
        ...
        // check missed all 20 of them, reporting JAUNTYGEN003 against Extrode.Jaunty's own source.
    ...
    /// </para>
    /// <para>
    /// Measured, and the reason this exists: without it the diagnostic fired 170 times inside Extrode.Jaunty's
    ...
    /// was measured against the library: Extrode.Jaunty's own generic write plumbing hands the entity to
    ...
    /// the guess would fire six times inside Extrode.Jaunty on the benign one.
    /// </para>
    /// <para>
    ...
    /// <summary>Whether <paramref name="method"/> is one of Extrode.Jaunty's own APIs.</summary>
    /// <remarks>
    /// Checked on the namespace rather than the assembly name, so that Extrode.Jaunty's own test projects -
    /// whose assemblies are also called <c>Extrode.Jaunty.*</c> - do not match their own helpers.
    /// </remarks>
    private static bool IsJauntyMethod(IMethodSymbol method)
    {
        ...
        return ns is not null
            && (ns.Equals("Extrode.Jaunty", StringComparison.Ordinal) || ns.StartsWith("Extrode.Jaunty.", StringComparison.Ordinal));
    }
    ...
    private static bool IsNameableFrom(ITypeSymbol type)
    {
        ...
        // anonymous type, so `new { Ids = ids }` (the idiomatic shape for Extrode.Jaunty's own IN-clause
    ...
    private static string GenerateParameterRoots(ImmutableArray<ParameterRoot> roots)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("namespace Extrode.Jaunty.Generated");
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Tells the trimmer to keep the public properties of every type this assembly passes to a");
        sb.AppendLine("    /// Extrode.Jaunty API as a parameters object. Generated; see Extrode.Jaunty.JauntyAot and spec 011.");
        sb.AppendLine("    /// </summary>");
        ...
        foreach (string typeArgument in typeArguments)
            sb.AppendLine($"            global::Extrode.Jaunty.JauntyAot.PreserveParameters<{typeArgument}>();");
        ...
        foreach (string witness in witnesses)
        {
            ...
            sb.AppendLine($"            global::Extrode.Jaunty.JauntyAot.PreserveParameters({witness});");
        }
