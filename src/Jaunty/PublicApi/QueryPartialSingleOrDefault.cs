...
public static partial class Jaunty
{
    ...
    {
        public T? QueryPartialSingleOrDefault<T>(string sql) where T : new()
        {
            return QuerySingleOrDefaultCore<T>(connection, sql, null, default, MappingMode.Projection);
        }

        public T? QueryPartialSingleOrDefault<T>(string sql, object parameters) where T : new()
        {
            return QuerySingleOrDefaultCore<T>(connection, sql, parameters, default, MappingMode.Projection);
        }

        public T? QueryPartialSingleOrDefault<T>(string sql, CommandOptions<T> options) where T : new()
        {
            return QuerySingleOrDefaultCore<T>(connection, sql, null, options, MappingMode.Projection);
        }

        public T? QueryPartialSingleOrDefault<T>(string sql, object parameters, CommandOptions<T> options) where T : new()
        {
