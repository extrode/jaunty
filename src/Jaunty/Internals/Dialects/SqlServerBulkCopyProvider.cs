...
internal sealed class SqlServerBulkCopyProvider : IBulkCopyProvider
{
    ...
    public int CopyToServer(IDbConnection connection, string tableName, IDataReader data, BulkCopyOptions options)
    {
        ...
        // Get SqlConnection type
        var sqlConnectionType = connection.GetType();
        if (!sqlConnectionType.FullName!.Contains("SqlConnection"))
        ...
        // Handle transaction via reflection
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sqlConnectionType = connection.GetType();
        if (!sqlConnectionType.FullName!.Contains("SqlConnection"))
            throw new ArgumentException("Connection must be be a SqlConnection.", nameof(connection));
    ...
    private static object MapBulkCopyOptions(BulkCopyOptions options)
    {
        ...
        // Map FireTriggers option (if CheckConstraints is true, we might want to fire triggers)
        // This is a simplified mapping - in production you might want more granular control
