...
public static partial class Jaunty
{
    ...
    {
        public T QuerySingle<T>(string sql) where T : new()
        {
            return QuerySingleCore<T>(connection, sql, null, default, MappingMode.Strict);
        }

        public T QuerySingle<T>(string sql, object parameters) where T : new()
        {
            return QuerySingleCore<T>(connection, sql, parameters, default, MappingMode.Strict);
        }

        public T QuerySingle<T>(string sql, CommandOptions<T> options) where T : new()
        {
            return QuerySingleCore<T>(connection, sql, null, options, MappingMode.Strict);
        }

        public T QuerySingle<T>(string sql, object parameters, CommandOptions<T> options) where T : new()
        {
