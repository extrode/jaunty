...
public static partial class Jaunty
{
    extension(IDbConnection connection)
    {
        ...
        public T? QueryFirstOrDefault<T>(string sql, CommandOptions<T> options) where T : new()
        {
            return QueryFirstOrDefaultCore<T>(connection, sql, null, options, MappingMode.Strict);
        }

        public T? QueryFirstOrDefault<T>(string sql, object parameters, CommandOptions<T> options) where T : new()
        {
