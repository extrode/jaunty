...
public static partial class Jaunty
{
    ...
    public static IEnumerable<T> QueryStream<T>(this IDbConnection connection, string sql, CommandOptions options) where T : new()
    {
        return QueryStreamCore<T>(connection, sql, null, options, MappingMode.Strict);
    }

    public static IEnumerable<T> QueryStream<T>(this IDbConnection connection, string sql, object parameters, CommandOptions options) where T : new()
    {
        return QueryStreamCore<T>(connection, sql, parameters, options, MappingMode.Strict);
    }

    public static IEnumerable<T> QueryStream<T>(this IDbConnection connection, string sql, object param1, object param2, params object[] rest) where T : new()
    {
        return QueryStreamCore<T>(connection, sql, CombineParams(param1, param2, rest), default, MappingMode.Strict);
    }
}
