...
public sealed partial class DuckDb
{
    ...
    {
        return await DeleteCoreAsync(predicate, default, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        return await DeleteCoreAsync(predicate, options, cancellationToken).ConfigureAwait(false);
    }

    private async ValueTask<int> DeleteCoreAsync<T>(Expression<Func<T, bool>> predicate, CommandOptions options, CancellationToken cancellationToken) where T : class, new()
    {
        ...
        var result = await NonQueryExecutor.ExecuteAsync(_connection, sql, whereParams, options, cancellationToken).ConfigureAwait(false);
