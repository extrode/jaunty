...
internal sealed class WhereExpressionVisitor<T> : ExpressionVisitor where T : new()
{
    ...
    protected override Expression VisitMethodCall(MethodCallExpression node)
    {
        ...
        // captured state - a subtree that still references the lambda parameter cannot be
        // compiled, and used to surface as an opaque "variable 'p' ... is not defined" from
        // ExpressionEvaluator. That is how the C# 14 rebinding above went unrecognised: an
        // unsupported translation looked like an evaluation bug. Name the method instead.
        if (ReferencesLambdaParameter(node))
        {
            throw new NotSupportedException(
                $"Cannot translate '{node.Method.DeclaringType?.FullName}.{node.Method.Name}' to SQL: it is applied to " +
                "the queried entity rather than to captured values, and Jaunty.Fluent has no SQL equivalent for it. " +
                "Rewrite the predicate using a supported operator, or filter in memory after the query.");
        }
    ...
    /// <summary>
    /// True when any part of <paramref name="expression"/> references a lambda parameter, which
    /// makes it impossible to evaluate as a captured constant.
    /// </summary>
