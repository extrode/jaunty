...
Executed as planned, plus one unplanned fix: the rename script's `.cs`-file discovery
(`find src tests tools benchmarks -name "*.cs"`) omitted `samples/`, so 176 `.cs` files under
`samples/NativeAOT-*` and `samples/torture-test-*` still had bare `using Jaunty;` after the first
pass — fixed with the same safe regex extended to `samples/`. Also required using the compiler as
an oracle for cascading regex bugs caused by the static class `Jaunty` sharing a name with its
enclosing namespace `Jaunty` (see commit history / CHANGELOG `[Unreleased]` entry for the
consumer-facing summary).

Final verification: `dotnet build Jaunty.slnx -c Release --no-incremental` — 0 errors, 175
warnings (pre-existing CS8767/CS8766 nullability, unrelated to this change). Full test suite:
27,329 total, 27,255 passed, 0 failed, 74 skipped (pre-existing DB-not-configured integration
skips). `git grep` sweep for un-prefixed `Jaunty` found only expected survivors: the 82
`public static partial class Jaunty` declarations, their call sites relying on C# namespace
walk-up resolution, and literal `Jaunty.slnx` filename references.
