# Jaunty Tests Reorganization Report
...
The test project now mirrors the Jaunty source structure:
```
tests/Jaunty.Tests/
...
### Integration Tests (Integration/Read/)
- **`QueryTests.cs`** - Tests for Query<T> methods
- **`QueryAsyncTests.cs`** - Tests for QueryAsync<T> methods
- **`QueryScalarTests.cs`** - Tests for QueryScalar<T> methods
- **`QueryScalarAsyncTests.cs`** - Tests for QueryScalarAsync<T> methods
- **`QueryPartialAsyncTests.cs`** - Tests for QueryPartialAsync<T> methods
- **`NamedParameterBindingTests.cs`** - Tests for named parameter binding
- **`PositionalParameterBindingTests.cs`** - Tests for positional parameter binding
- **`QueryPositionalParameterTests.cs`** - Tests for positional parameters in Query methods
- **`QueryEdgeCaseTests.cs`** - Tests for edge cases in Query methods
- **`QueryMappingModeTests.cs`** - Tests for strict vs partial mapping modes
- **`QueryAttributeMappingTests.cs`** - Tests for attribute-based mapping
- **`QueryErrorHandlingTests.cs`** - Tests for error handling in Query methods
- **`QueryDatabaseConnectionTests.cs`** - Tests for database connection handling

### Unit Tests (Unit/Read/)
- **`CommandOptionsTests.cs`** - Tests for CommandOptions functionality
- **`ParameterBinderTests.cs`** - Tests for parameter binding logic
- **`SqlParameterParserTests.cs`** - Tests for SQL parameter parsing

### Integration Tests (Other Directories)
- **`Integration/Multiple/QueryMultipleTests.cs`** - Tests for QueryMultiple functionality
- **`Integration/Configuration/ConfigResolverTests.cs`** - Tests for configuration resolution

## Key Improvements

### 1. Specificity
Each test file now focuses on a specific API method or functionality, making it easier to locate relevant tests.
