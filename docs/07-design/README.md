...
`tools/Extrode.Jaunty.DocsGenerator` is the console app that implements this design as a static site generator. Run it with:

```bash
dotnet run --project tools/Extrode.Jaunty.DocsGenerator -- <input-dir> <output-dir>
# Example:
dotnet run --project tools/Extrode.Jaunty.DocsGenerator -- docs/ dist/docs-site/
```

See `tools/Extrode.Jaunty.DocsGenerator/Program.cs` for the CSS and JS implementation of these tokens.
