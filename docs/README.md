...
`scripts/build-docs.sh` runs the shared `Docs` tool (sibling repo `../docs`), which converts this markdown tree into a self-contained static HTML site matching the design in `07-design/`. Page layout, CSS, and nav JS are generated by the tool itself, not read from this repo. The tool is not part of this repository, so the script runs only for maintainers with that sibling checkout; the rendered site is committed under `dist/docs-site/`.
