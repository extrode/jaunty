...
- **[_assets/site-design/](_assets/site-design/README.md)**
  - [`jaunty-docs-app.dc.html`](_assets/site-design/jaunty-docs-app.dc.html) - Primary design (authoritative layout target)
  - [`jaunty-docs.dc.html`](_assets/site-design/jaunty-docs.dc.html) - Secondary design reference
  - [`support.js`](_assets/site-design/support.js) - Design-tool runtime (view `.dc.html` files in browser)
...
`scripts/build-docs.sh` runs the shared `Docs` tool (sibling repo `../docs`), which converts this markdown tree into a self-contained static HTML site matching the design in `_assets/site-design/`. Page layout, CSS, and nav JS are generated by the tool itself, not read from this repo. The tool is not part of this repository, so the script runs only for maintainers with that sibling checkout; the rendered site is committed under `dist/docs-site/`.
...
**Design tokens** and implementation details are documented in [`_assets/site-design/README.md`](_assets/site-design/README.md).
