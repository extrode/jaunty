...
`scripts/build-docs.sh` runs the shared `Beparey.DocsGen` tool (sibling repo `../docsgen`), which converts this markdown tree into a self-contained static HTML site matching the design in `07-design/`. Page layout, CSS, and nav JS are generated by the tool itself, not read from this repo.
