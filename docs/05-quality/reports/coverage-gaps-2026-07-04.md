...
1. `Extrode.Jaunty.Fluent.JoinedQuery4Builder<T1..T4>` — 0/164 lines, and
   `Extrode.Jaunty.Fluent.JoinClause4Builder<T1..T4>` — 0/43. The whole 4-entity join
   surface is untested (relates to PRD-007 3-way/4-way join parity).
2. `Extrode.Jaunty.Fluent.Internals.QueryBuilderBase` — 0/72.
3. `Extrode.Jaunty.JauntyLoggingExtensions` — 0/34 (public logging API, shipped in core).
4. `Extrode.Jaunty.TypeHandlers.TypeHandlerRegistry` — 42% (15/36); Remove/replace paths
   untested.
5. `Extrode.Jaunty.FlatFiles.FileSources.ExcelFileSource` (0/33),
...
6. `Extrode.Jaunty.FlatFiles.Core.FlatFileOptions` — 38% (28/73).
7. `Extrode.Jaunty.Scaffolding.Providers.MySql.MySqlTypeMapper` (0/53) and
...
8. `Extrode.Jaunty.FlatFiles.DuckDB.Internals.Import.ImportDialectResolver` — 30% (6/20).
9. `Extrode.Jaunty.Extensions.Reflection.MetadataCache<T>` — 50% (77/155).
10. `Extrode.Jaunty.Jaunty` (core static API surface) — 42% (419/1000): large #if-net472/
...
- `Extrode.Jaunty.Extensions.Reflection.BulkCopy.{MySql,PostgreSql,SQLite,SqlServer}BulkCopyProvider`
...
- `Extrode.Jaunty.FlatFiles.DuckDB.Internals.Import.{PostgreSql,SqlServer}ImportDialect`.
- `Extrode.Jaunty.Scaffolding.Providers.{MySql,PostgreSql,SqlServer}SchemaReader`.
...
- MySQL table-state isolation in `Extrode.Jaunty.Tests` Release mode (47 failures when run
