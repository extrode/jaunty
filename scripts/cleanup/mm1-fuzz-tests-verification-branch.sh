#!/usr/bin/env bash
# Written 2026-09-17 by Claude after verifying the adversarial-input parser/SQL-builder tests
# (test/fuzz-parsers-and-sql-builders) on mm1 via a git-bundle transfer. Removes the throwaway
# verification branch and bundle file left behind on mm1 once the tests are safely merged into dev
# on the primary Windows checkout. Run ON mm1, not on Windows.
#
# Dry run (default):  bash mm1-fuzz-tests-verification-branch.sh
# Apply:               bash mm1-fuzz-tests-verification-branch.sh --execute
set -eu

REPO="$HOME/Developer/code/extrode.com/jaunty"
BRANCH="test/fuzz-parsers-and-sql-builders"
BUNDLE="$HOME/jaunty-fuzz-tests.bundle"
