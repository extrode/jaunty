#!/usr/bin/env bash
# Written 2026-09-17 by Claude after verifying the ConfigurationGeneration/CrudSqlCache
# concurrency stress test (test/concurrency-stress-configuration-generation) on mm1 via a
# git-bundle transfer. Removes the throwaway verification branch and bundle file left behind
# on mm1 once the branch is merged into dev on the primary Windows checkout. Run ON mm1, not
# on Windows.
#
# Dry run (default):  bash mm1-concurrency-stress-verification-branch.sh
# Apply:               bash mm1-concurrency-stress-verification-branch.sh --execute
set -eu

REPO="$HOME/Developer/code/extrode.com/jaunty"
BRANCH="test/concurrency-stress-configuration-generation"
BUNDLE="$HOME/concurrency-stress.bundle"
