...
namespace Extrode.Jaunty.FlatFiles.DuckDB.Tests.Integration;

/// <summary>
/// M0 Exit Gate Tests: Proves that Extrode.Jaunty's materialization pipeline works correctly
...
public class JauntyMaterializationTests : IDisposable
{
    ...
    {
        // Register the DuckDB dialect so Extrode.Jaunty's SqlDialectFactory can resolve it
    ...
    {
        // M0 EXIT GATE: Extrode.Jaunty's QueryPartial<T> works with DuckDB
    ...
    {
        // Load CSV file as a DuckDB view, then materialize via Extrode.Jaunty
