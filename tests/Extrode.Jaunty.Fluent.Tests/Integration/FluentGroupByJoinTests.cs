...
/// MariaDB) coverage lives in Extrode.Jaunty.Fluent.SourceGen.Tests, which already has the
...
public class FluentGroupByJoinTests : IClassFixture<FluentDatabaseFixture>
{
    ...
    [Fact]
    public void GroupBy_Having_NotEqual_FiltersGroups()
    {
        // coverage-gaps-2026-09-20: only > and < were exercised anywhere for joined HAVING.
        // Category 1 has 4 products, category 2 has 5, category 3 has 2.
        var results = _fixture.Connection.From<Product>()
            .InnerJoin<Category>()
            .On(p => p.CategoryId, c => c.CategoryId)
            .GroupBy((p, c) => p.CategoryId)
    ...
    [Fact]
    public void GroupBy_Having_GreaterThanOrEqual_FiltersGroups()
    {
        var results = _fixture.Connection.From<Product>()
            .InnerJoin<Category>()
            .On(p => p.CategoryId, c => c.CategoryId)
    ...
    [Fact]
    public void GroupBy_Having_LessThanOrEqual_FiltersGroups()
    {
        var results = _fixture.Connection.From<Product>()
            .InnerJoin<Category>()
            .On(p => p.CategoryId, c => c.CategoryId)
    ...
    [Fact]
    public void GroupBy_Having_UnsupportedOperator_Throws()
    {
        // `&` on bool operands is a non-short-circuit ExpressionType.And, not AndAlso -- the
        // joined operator switch has no arm for it and falls to the NotSupportedException default.
