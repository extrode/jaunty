...
/// the real code with no server: the assertions here are about what Extrode.Jaunty writes to the copy
/// stream and what it does to that stream when the read fails, both of which are Extrode.Jaunty's alone.
/// </para>
/// <para>
...
public class CsvImportPostgreSqlStdinTests : IDisposable
{
    ...
    /// completes the COPY on Dispose, so unless Extrode.Jaunty cancels it the rows written before the
