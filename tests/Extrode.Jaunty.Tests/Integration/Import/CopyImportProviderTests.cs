...
public class CopyImportProviderTests : IDisposable
{
    ...
    private static string FindCsvImportSource()
    {
        ...
        {
            string candidate = Path.Combine(dir.FullName, "src", "Extrode.Jaunty", "Import", "CsvImport.cs");
            if (File.Exists(candidate))
                return candidate;
            dir = dir.Parent;
        }

        throw new FileNotFoundException("src/Extrode.Jaunty/Import/CsvImport.cs was not found above the test binary.");
    }
}
