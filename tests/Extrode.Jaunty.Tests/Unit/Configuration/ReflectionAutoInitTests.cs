...
/// <summary>
/// AUD-R35-099. <c>Extrode.Jaunty</c>'s static constructor calls <c>TryEnableReflectionMapping</c>, which
/// invokes <c>UseReflectionMapping()</c> reflectively. That constructor does not run at startup: it
/// runs on the first touch of any <c>Extrode.Jaunty</c> static member, so a caller who configures
...
/// <see cref="Extrode.Jaunty.Tests.Unit.Configuration.ConfigurationGenerationTests"/>: both mutate the
...
public class ReflectionAutoInitTests
{
    ...
    public void AutoInit_DoesNotOverwriteAResolverTheCallerAlreadySet()
    {
        ...
        try
        {
            global::Extrode.Jaunty.Jaunty.TryEnableReflectionMapping();
