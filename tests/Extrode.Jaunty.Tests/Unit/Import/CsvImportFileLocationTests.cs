...
/// machine is exactly what <c>BULK INSERT</c> is for, and Extrode.Jaunty rejected it with
...
public class CsvImportFileLocationTests
{
    ...
    /// filesystem, so a path that does not exist here is not an error Extrode.Jaunty can diagnose - it has
    ...
    /// <summary>
    /// SQLite reads the file itself, so a missing file is something Extrode.Jaunty can and should diagnose
    ...
    private sealed class SqlConnection : IDbConnection
    {
        ...
        /// tests is that Extrode.Jaunty wraps whatever the provider threw and keeps it as the inner
        /// exception.
        /// </summary>
