...
/// <summary>
/// AUD-R26 (batch 4 / 5 / 7, filed four times, fixed as one cluster). Whether a Extrode.Jaunty command is
...
/// "Extrode.Jaunty" diagnostic source without registering an <see cref="ICommandInterceptor"/> would emit
/// events. It never took effect for a single ordinary Extrode.Jaunty API, because every call site returned
...
public class InterceptorObservabilityTests : IDisposable
{
    ...
    private void AssertSawACommand(string what)
    {
        Assert.True(
            ...
            $"A subscriber to the 'Extrode.Jaunty' diagnostic source with no ICommandInterceptor registered " +
    ...
    private static DirectoryInfo LocateSourceRoot()
    {
        ...
        {
            string candidate = Path.Combine(dir.FullName, "src", "Extrode.Jaunty", "Interceptors", "InterceptorPipeline.cs");
