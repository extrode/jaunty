...
namespace Extrode.Jaunty.Tests.Unit.StoredProcedures;
...
/// <em>Extrode.Jaunty itself</em> called those overloads with null: the stored-procedure family declares
...
///
/// <para>
/// These tests need no server. The exception was raised at Extrode.Jaunty's own argument check, before
...
public class NoParameterOverloadTests
{
    ...
    /// what is being asserted. What must not happen is Extrode.Jaunty rejecting its own forwarded null
    ...
    private static void AssertReachesTheProvider(Action call)
    {
        ...
        {
            // Anything else means Extrode.Jaunty passed the argument along and the provider objected,
