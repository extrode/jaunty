...
/// as a passing run. <c>tests/Extrode.Jaunty.Fluent.SourceGen.Tests</c> was outside the solution from the
/// day spec 003 created it, and neither it nor <c>tests/Extrode.Jaunty.Scaffolding.Cli.Tests</c> had a CI
...
public class SolutionLayoutTests
{
    ...
    public void EveryTestProjectInTheSolutionIsInvokedByBothCiFrameworkLegs()
    {
        ...
        //   - VSTest: "FullyQualifiedName~Extrode.Jaunty.FlatFiles" (substring on the fully qualified name)
        //   - xunit v3 / Microsoft.Testing.Platform: --filter-namespace "Extrode.Jaunty.FlatFiles*"
        // Both match on a namespace-prefix token, so "Extrode.Jaunty.FlatFiles" covers both FlatFiles
        // suites - and, exactly as this bug went, "Extrode.Jaunty.Fluent.Tests" covers Extrode.Jaunty.Fluent.Tests
